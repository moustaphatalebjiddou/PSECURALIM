...
use App\Exports\ComplementExport;
...
class ComplementController extends Controller
{
    ...
    $complements = Complement::when($search, function($query, $search) {
        return $query->where(function ($q) use ($search) {
            // Rechercher par les colonnes de complement
   ...
   //     $complement = Complement::all();
        return view('backend.complement.index-complement', compact('complements'));
    }

    public function create()
    {
        $wilayas = Wilaya::all();
        return view('backend.complement.add-complement', compact('wilayas'));
    }
    ...
    public function store(Request $request)
    {
       // dd($request->all()); // Pour voir les données envoyées
        ...
        return redirect()->route('complement.index')->with('success', 'complement Irriguée ajoutée avec succès');
    }


                    public function edit($id)
                {
                    // Récupérer les informations de l'complement Irriguée à éditer
                    $complement = Complement::find($id);

                    if (!$complement) {
                        return redirect()->route('complement.index')->with('error', 'complement Irriguée non trouvée.');
                    }

                    $wilayas = Wilaya::all();

                    return view('backend.complement.edit-complement', compact('complement', 'wilayas'));
                }
                ...
                public function update(Request $request, $id)
                {
                    ...
                    // Récupérer l'complement Irriguée
                    $complement = Complement::find($id);

                    if (!$complement) {
                        return redirect()->route('complement.index')->with('error', 'complement non trouvée.');
                    }

                    // Mise à jour des données
                    $complement->update($request->all());

                    return redirect()->route('complement.index')->with('success', 'complement mise à jour avec succès.');
                }
                ...
                public function destroy($id)
                {
                    ...
                    return redirect()->route('complement.index')->with('success', 'L\'complement irriguée a été supprimée avec succès.');
                }
    ...
    public function export()
    {
       ...
       return Excel::download(new ComplementExport, 'Complement.xlsx');
    }
    ...
    public function exportPdf() {
        ...
        if (!view()->exists('backend.complement.pdf')) {
            return response()->json(['error' => 'La vue PDF n\'existe pas.'], 404);
        }

        $pdf = PDF::loadView('backend.complement.pdf', compact('complements'));
        return $pdf->download('complement.pdf');
    }

    public function print_complement($id){
        // Récupération de l'complement par son ID
        $complement = Complement::where('id', $id)->first();

        // Vérifiez si l'complement a été trouvé
        if (!$complement) {
            return response()->json(['error' => 'complement.'], 404);
        }

        // Passez la variable correcte à la vue
        return view('backend.complement.print-complement', compact('complement'));
    }
